...
export const FormUser = (props: PropsFormUser) => {
  ...
  return (
    <form method="post" className="mt-5" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className={'container-layout flex justify-between' }  onClick={() => setShowStep1(!showStep1)}>
        <label className={'text-gray-700 cursor-pointer'}> Datos personales</label>
        <TiArrowUp className={` animation-arrow cursor-pointer ${showStep1 && 'rotate-180'}`} size={32} />
      </div>
      <div className={` content-step ${showStep1? 'collapse-step-1' : 'expand-step-1'}`}>
        <div className={`flex`}>
      ...
      {/* Paso 2 - Educación */}
      <div className="mt-5">
        <div className={'container-layout flex justify-between' }  onClick={() => setShowStep2(!showStep2)}>
          <label className={'text-gray-700 cursor-pointer'}>Educación</label>
          <TiArrowUp className={` animation-arrow cursor-pointer ${showStep2 && 'rotate-180'}`} size={32} />
        </div>
        <div className={` content-step ${showStep2? 'collapse-step-1' : 'expand-step-2'}`}>
      ...
      <div className="mt-3">
        <div className={'container-layout flex justify-between' }  onClick={() => setShowStep3(!showStep3)}>
          <label className={'text-gray-700 cursor-pointer'}>Empleabilidad</label>
          <TiArrowUp className={` animation-arrow cursor-pointer ${showStep3 && 'rotate-180'}`} size={32} />
        </div>
        <div className={` content-step ${showStep3? 'collapse-step-1' : 'expand-step-2'}`}>
          <div className="grid grid-cols-2">
      ...
      {/* Paso 4 - Registro */}
      <div className="mt-3">
        <div className={'container-layout flex justify-between' }  onClick={() => setShowStep4(!showStep4)}>
          <label className={'text-gray-700 cursor-pointer'}>Registro</label>
          <TiArrowUp className={` animation-arrow cursor-pointer ${showStep4 && 'rotate-180'}`} size={32} />
        </div>
        <div className={` content-step ${showStep4? 'collapse-step-1' : 'expand-step-2'}`}>
          <div className="grid grid-cols-3">
            <input
              type="text"
